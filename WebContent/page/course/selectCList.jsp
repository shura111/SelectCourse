			...
			<c:forEach var="o" items="${courselist }">
				...
				<tr>
					...
					<c:forEach var="p" items="${list2 }">
						<c:if test="${o.getcourse_id() eq p.getcourse_id() }">
					...
					<c:if test="${f eq 0 }">
						<c:if test="${o.getcourse_sum() ne 0 }">
							<td><input type="checkbox" name="switch" value="${o.getcourse_id() }" lay-skin="switch" lay-text="已选|未选" lay-filter="check"></td>
						</c:if>
						<c:if test="${o.getcourse_sum() eq 0 }">
							<td><input type="checkbox" name="switch" value="${o.getcourse_id() }" disabled="disabled" lay-skin="switch" lay-text="已选|未选" lay-filter="check"></td>
						</c:if>
					</c:if>
				</tr>
				</tbody>
		  ...
		  form.on('switch(check)', function(data){
			  ...
			  layer.open({
	                ...
	                ,yes: function(index, layero){
	                    ...
	                    layer.close(index);
	                    $.ajax({
	                		url : "${pageContext.request.contextPath}/StuchooseServlet",
	                		type : "POST",
	                		async : true,
	                		data :	{"state" : x,"id": id},
	                		dataType : "text",
